...
import {
  ...
  postNewTableFn,
  putNewTableFn,
} from "@api/admin/admin.restraunt.scheme.save.ts";
  ...
  return (
    <>
      <Flex gap={50}>
        ...
        {isSelected && (
          <Input
            value={tables[selectIdinArray].numberOfPeople}
            placeholder={"Вместитмость"}
            type={"number"}
            onChange={(e) =>
              saveTable(
                tables[selectIdinArray],
                selectIdinArray,
      ...
      <Stage
        ...
        height={700}
        width={900}
        style={{ background: "#D0EBFF", width: "900px" }}
      >
        <Layer>
          {tables.map((table, i) =>
            table.type === "rect" ? (
              <Rect
                ...
                onClick={() => {
                  setSelectId(table.id),
                    setIsSelected(true),
                    setSelectIdinArray(i);
                }}
